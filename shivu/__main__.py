...
async def message_counter(update: Update, context: CallbackContext) -> None:
    ...
    async with lock:
        ...
        if chat_frequency:
            message_frequency = chat_frequency.get('message_frequency', 100000)
        else:
...
async def send_image(update: Update, context: CallbackContext) -> None:
    ...
    # Count the total number of Limited Edition characters spawned today
    total_limited_spawned = sum(1 for char_id in sent_characters[chat_id] if any(char.get('rarity') == '🔮 Limited Edition' for char in all_characters if char.get('id') == char_id))
    ...
    spawn_counts = {
        '⚪️ Common': 1,  # Common always spawns
        '🟣 Rare': 1,     # Rare always spawns
        '🟡 Legendary': 2,  # Legendary always spawns
        '🟢 Medium': random.randint(0, 1),  # Medium spawns randomly
        '💮 Special Edition': 3,  # Special Edition spawns 6 times in a day
        '🔮 Limited Edition': 2 if total_limited_spawned < 2 else 0,  # Limited Edition spawns 2 times in a day        
        '🎐 Celestial': 1 if datetime.datetime.today().weekday() in [0, 7] else 0,  # Christmas Special never spawns
    }
    ...
    if today_message_count <= 6:  # Special Edition can spawn up to 4 times in a day
